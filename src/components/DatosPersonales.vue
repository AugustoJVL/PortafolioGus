...
<template>
  <section class="datos-personales">
    <div class="card">
      <h1>{{title}}</h1>
      <a class="icono-grande">
        <img class="logo-grupo" :src="logo[0].src" alt="Logo">
      </a>
      <h2>{{descripcion}}</h2>
      <p>{{presentacion}}</p>
      <ul class="container-lista">
        <li v-for="red in redes" :key="red.id">
          <a :href="red.url"><img class="icon-redsocial" :src="red.src" width="35rem" alt="Redes"></a>
        </li>
      </ul>
...
<style scoped>
  .datos-personales{
    background-color: rgba(87, 26, 122, 0.747);
    border-radius: 5px;
    margin: 1rem;
  }
  .card{
  ...
  .logo-grupo {
    width: 100%; /* % del tamaño del logo dentro del círculo */
    border-radius: 50%;
  }
  ...
  .container-lista{
    ...
    li{
     background-color:  rgba(132, 0, 255, 0.432);
     border-radius: 100%;
     height: 2.5rem;
     width: 2.5rem;
     margin: .3rem;
